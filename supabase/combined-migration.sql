...
CREATE POLICY "public read approved" ON public.reviews FOR SELECT USING (is_approved = true);
CREATE POLICY "admin read all reviews" ON public.reviews FOR SELECT TO authenticated USING (public.has_role(auth.uid(),'admin'));
CREATE POLICY "anyone submit review" ON public.reviews FOR INSERT WITH CHECK (is_approved = false);
...
GRANT SELECT ON public.room_products TO anon, authenticated;
GRANT ALL ON public.room_products TO service_role;
GRANT INSERT, UPDATE, DELETE ON public.room_products TO authenticated;
ALTER TABLE public.room_products ENABLE ROW LEVEL SECURITY;
CREATE POLICY "public read room_products" ON public.room_products FOR SELECT USING (true);
CREATE POLICY "admin write room_products" ON public.room_products FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin')) WITH CHECK (public.has_role(auth.uid(),'admin'));

-- Security hardening: block anon from calling has_role directly; authenticated needs EXECUTE for RLS policies.
